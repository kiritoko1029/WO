...
Debian 构建依赖默认依次尝试中科大、清华和 Debian 官方镜像。下载有超时与重试限制，任何索引下载失败都会停止该次安装并切换来源，不会继续使用残缺索引。基础 slim 镜像尚无 CA 包，初始 APT 下载使用 HTTP，但始终验证 Debian 官方签名与包校验和。

### 首次部署失败后拉取修复

若首次构建失败（例如原来的 `snapshot.debian.org` 超时），修复代码发布后执行：

```bash
git pull --ff-only
bash deploy.sh up --refresh-build
```

使用了 `--project` 或 `--state-dir` 时继续附带原参数。此选项仅刷新**尚未完成首次安装**的构建版本，不改变域名、IP、部署身份、密码或密钥，也不删除数据库或证书卷。已完成安装会拒绝此选项；它不是通用升级命令。不要为了重试而删除 `deploy/.managed/`。

Docker 基础镜像和 npm 包的下载是另外两条网络链路。若报错来自 Docker Hub、GHCR 或 npm registry，请检查服务器到对应服务的连接，按云服务商文档配置可信的镜像加速；APT 镜像修复不会替代 Docker 的网络配置。
